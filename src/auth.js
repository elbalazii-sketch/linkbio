...
function Adapter(client) {
  return {
    ...
    async createUser(user) {
      const { name, email, emailVerified, image, is_admin } = user;
      const sql = `
        INSERT INTO auth_users (name, email, "emailVerified", image, is_admin)
        VALUES ($1, $2, $3, $4, $5)
    ...
    async updateUser(user) {
      ...
      const { id, name, email, emailVerified, image, is_admin } = newUser;
      const updateSql = `
        UPDATE auth_users set
        name = $2, email = $3, "emailVerified" = $4, image = $5, is_admin = $6
        where id = $1
        RETURNING name, id, email, "emailVerified", image, is_admin
      `;
